...
async function getCollectiblesArticles(): Promise<Article[]> {
  const contentDir = path.join(process.cwd(), 'src/content/collectibles')
...
export default async function CollectiblesPage() {
  const articles = await getCollectiblesArticles()
    ...
    <div className="container mx-auto py-12">
      ...
      <div className="flex flex-col justify-between items-center mb-6">
        <h1 className="mx-auto max-w-3xl text-3xl font-bold lg:text-5xl tracking-tight text-white drop-shadow-lg">
          <span className="pt-10">Collectibles</span>
        </h1>
        <h2 className="mx-auto max-w-[700px] md:text-xl my-6 text-gray-200 drop-shadow-md">
          Find all hidden items, scrolls, and rare collectibles in Where Winds Meet
        </h2>
      </div>
          ...
          {articles.map((article, index) => (
            ...
            >
              <div className="flex flex-col space-y-1.5 p-6">
                <Link
                  href={`/collectibles/${article.slug}`}
